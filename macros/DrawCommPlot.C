...
void Draw(TString name, TString histotitle, bool log, bool doData) {
  ...
  hist_c                = (TH1F*)myFile->Get("QCD__"+name+"_c");
  hist_gsplit           = (TH1F*)myFile->Get("QCD__"+name+"_bfromg");
  hist_l                = (TH1F*)myFile->Get("QCD__"+name+"_l");
  ...
  //Legend
  if (doData) qw->AddEntry(hist_data,     datacaption,                       "p");
...
void DrawMC(TString name, TString histotitle, bool log) {
  ...
  hist_data     = (TH1F*)myFile->Get(name+"_data");
  float scale_f = (hist_data->Integral())/(hist_mc->Integral());
  //Already scaled hist_mc       ->Scale(scale_f);
