...
void DrawAll(bool Draw_track_plots, bool Draw_Nminus1_plots, bool Draw_sv_plots, bool Draw_muons_plots, bool Draw_discriminator_plots, bool Draw_tagRate_plots, bool Draw_2D_plots, TString histoTag) {
  ...
  DrawStacked(histoTag+"_pruned_mass" ,"pruned mass all jets"      ,1 ,1);
  ...
  if (Draw_track_plots){
    ...
    DrawStacked(histoTag+"_track_pt"      ,    "pT of all the tracks",1,1);
  ...
  if (Draw_Nminus1_plots){
    ...
    DrawStacked(histoTag+"_track_pt_cut"	    ,"Track pT @N-1 step",1,1);
  }
  if (Draw_sv_plots){
    ...
    DrawStacked(histoTag+"_sv_deltaR_jet","Delta R between the jet and the SV direction.",0,1);
    DrawStacked(histoTag+"_sv_deltaR_sumJet","#Delta R between the jet and the SV",0,1);
    DrawStacked(histoTag+"_sv_deltaR_sumDir","#Delta R between the jet direction and the SV",0,1);
